...
I’m currently a 2nd-year Mechanical Engineering student at the University of Waterloo, specializing in mechatronics. I’ve been actively involved with a student design team called Watonomous, where I lead the perception team in developing algorithms and solutions for autonomous vehicles. My experience includes working with advanced technologies like 3D object tracking, deep learning for behavioral cloning, and computer vision, particularly using ROS2 and YOLOv8 for object detection.

During my recent internship, I developed and integrated a full perception stack, including 3D tracking and object detection systems for autonomous driving. I’ve also built sensor racks and implemented real-time data processing for these systems. In addition to technical experience, I’ve had opportunities to mentor team members and contribute to optimizing our perception pipeline for autonomous vehicles.

I’m passionate about applying cutting-edge technologies like neural networks and computer vision in practical scenarios, which is why this role, combining Neural Radiance Fields (NeRF) with trajectory tracking for construction machinery, excites me. I’m particularly drawn to the opportunity to work with real-world models and advanced GPU processing, which aligns well with my technical background and aspirations in autonomous systems.

### Why do you want to work here
I’m particularly excited about this role because it aligns with my passion for autonomous systems and advanced technologies like Neural Radiance Fields (NeRF). The construction industry is undergoing a transformation with automation, and the chance to contribute to that innovation really excites me.

My previous experience in developing perception systems for autonomous vehicles has given me a solid foundation in 3D tracking, sensor fusion, and machine learning. Applying that knowledge to a new domain like construction machinery is a natural next step for me. The opportunity to explore NeRF, trajectory tracking, and collision avoidance is something I find intellectually stimulating, especially since I’m passionate about working on projects that push the boundaries of technology in practical applications.

Additionally, the strong research focus at the Institute for Geotechnical Engineering and Construction Management, as well as the chance to contribute to publications and conference presentations, aligns with my academic goals. I’m eager to contribute to a team that’s at the forefront of innovation and to learn from experts in the field while making a meaningful impact in construction automation.

### What is NeRF
Neural Radiance Fields (NeRF) is a machine learning technique that generates high-fidelity 3D representations of environments from 2D images. It works by training a neural network to predict the color and density of light rays at different positions and viewing angles in a 3D space. This neural network essentially learns to represent the scene using a 5D input vector, which consists of a 3D position and 2D viewing direction. The output is the color and volume density for that ray, allowing the network to synthesize realistic views of the scene from novel camera perspectives.

What makes NeRF particularly powerful is its ability to capture fine details and complex lighting effects in a scene. It uses techniques like positional encoding and hierarchical volume sampling to represent high-frequency details while keeping the file size smaller than traditional 3D models. This is particularly useful in environments where the geometry is complex or where capturing all the details manually would be time-consuming and resource-intensive.

In practical applications, like this research position, NeRF can be used to create detailed 3D environments from 2D images or video, which can then be integrated with trajectory optimization algorithms to navigate vehicles or machinery through that environment. This allows for accurate localization and collision avoidance, making it an exciting area for construction automation and robotics.

### How can NeRF help with trajectory tracking
NeRF can be used in trajectory tracking by providing highly accurate 3D reconstructions of environments, which serve as the foundation for planning and controlling the movement of a vehicle or machine through that space. Here's how it fits into trajectory tracking:

1. **3D Environment Reconstruction**: NeRF creates a high-fidelity 3D model of the environment from 2D images or video. This 3D representation provides a detailed map that includes obstacles, terrain, and other features, which is crucial for trajectory tracking and collision avoidance.
    
2. **Pretrained Environment Models**: NeRF can be pretrained offline to capture the specific environment where the tracked vehicle will operate. Once the environment is mapped, the NeRF model can be used in real-time to localize the vehicle within this 3D space.
    
3. **Localization and Path Planning**: By leveraging NeRF’s ability to generate accurate 3D views, the vehicle can identify its position in the environment using visual data. This enables trajectory tracking algorithms to compute optimal paths that avoid obstacles, taking into account the layout of the scene and any dynamic elements.
    
4. **Trajectory Optimization**: Once the vehicle is localized, trajectory optimization algorithms can use the NeRF environment to plan safe and efficient routes. Since NeRF models are highly detailed and accurate, they provide valuable input for the vehicle to plan precise trajectories. This includes adjusting for potential collisions or challenging terrain and ensuring the tracked vehicle follows the desired path.
    
5. **Collision Avoidance**: NeRF’s 3D environment helps identify obstacles in the path of the vehicle in real time. As the vehicle moves, trajectory tracking algorithms can continuously reference the NeRF model to ensure that its path avoids collisions, adjusting dynamically as needed.
    

In essence, NeRF provides the rich 3D data needed for trajectory tracking algorithms to perform localization, path planning, and collision avoidance. It allows vehicles to safely and efficiently navigate through complex environments, making it highly valuable in automation for industries like construction.
...
- What is your primary goal with this research project and how do you plan to use NeRF models for trajectory tracking?
